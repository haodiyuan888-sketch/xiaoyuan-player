...
function search163(kw){
  return get('https://music.163.com/api/search/get/web?csrf_token=&s='+encodeURIComponent(kw)+'&type=1&offset=0&total=true&limit=30',{},8000)
  .then(function(d){
...
function list163(pid){
  ...
  .then(function(d){
    if(!d||!d.playlist) throw new Error('网易歌单返回空: '+(d.code||'unknown'));
...
http.createServer(function(req,res){
  ...
  function serveFile(fp,ct){
    try{res.writeHead(200,ct);res.end(fs.readFileSync(fp))}
  ...
  if(pt==='/api/search'){
    ...
    SRC[s].search(kw)
      .then(function(v){send(200,v)})
      .catch(function(e){send(500,{error:'搜索失败: '+e.message,detail:String(e.stack||e)})})
    return
  }
  ...
  if(pt==='/api/playlist'){
    ...
    SRC[s].playlist(pid)
      .then(function(v){send(200,v)})
      .catch(function(e){send(500,{error:'歌单加载失败: '+e.message,detail:String(e.stack||e)})})
    return
  }
  ...
  if(pt==='/api/lyric'){
    ...
    SRC[s].lyric(sid)
      .then(function(v){send(200,{lyric:v})})
      .catch(function(e){send(500,{error:'歌词加载失败: '+e.message,detail:String(e.stack||e)})})
    return
  }
  ...
  if(pt==='/api/urls'){
    ...
    .then(function(r){
      send(200,{id:sid,source:s,urls:r.filter(function(x){return x.url&&x.url.startsWith('http')}),all:r})
    })
...
}).listen(PORT,function(){console.log('☀️ 小源启动 → 端口 ' + PORT);KEYS.forEach(function(k){console.log('  '+SRC[k].icon+' '+SRC[k].name)})});
