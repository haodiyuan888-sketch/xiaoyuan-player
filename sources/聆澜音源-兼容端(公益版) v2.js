...
const API_URL = "https://guoyue2010.seven-cloud.cn/api/music";
const API_KEY = "";
...
const checkUpdate = async () => {
  try {
    const requestRes = await httpFetch(`https://lc.guoyue2010.top/script?checkUpdate=${SCRIPT_MD5}&key=${API_KEY}`, {
      method: "GET",
      headers: {
        "Content-Type": "application/json",
        "User-Agent": `${env ? `lx-music-${env}/${version}` : `lx-music-request/${version}`}`,
